 ...
 */
shifter::shifter(int64_t num_samples) : num_samples(num_samples) {}

/**
 * Shift the input samples.
 * @param samples shared_ptr to sample buffer
 */
void shifter::process(shared_ptr<vector<complex<float>>> &samples,
                      int64_t metadata) {
  auto result = make_shared<vector<complex<float>>>(std::move(*samples));

  if (num_samples < 0) {
    if (abs(num_samples) <= (unsigned long int)result->size())
  ...
  SPDLOG_DEBUG(
      "Shifer block, shifting {} samples by num samples {}, metadata {}",
      samples->size(), num_samples, metadata);
