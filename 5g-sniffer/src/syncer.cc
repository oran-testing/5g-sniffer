...
syncer::syncer(uint64_t sample_rate, shared_ptr<nr::phy> phy)
    : sample_rate(sample_rate), phy(phy) {
  ...
  std::array<std::array<std::complex<float>, sss_length>, nid_max + 1> ssss =
      sss_ref.generate_all_sss_seq();
  phy->ssss = ssss;
  // Setup resampler
  unsigned int h_len = 51; // Filter semi-length (filter delay)
  resampling_rate = (float)phy->ssb_bwp->sample_rate /
                    (float)sample_rate; // Resampling rate (output/input)
  max_resampled_samples_per_sample = (unsigned int)ceilf(resampling_rate);
  float bw =
      0.08f; // Resampling filter bandwidth TODO this parameter is important
  ...
  pss_window_size =
      std::floor((float)sample_rate / (float)(phy->ssb_bwp->scs) * 8);

  // Create pool of 64 flows that can process samples in parallel after
  // synchronization
 ...
 */
syncer::~syncer() { resamp_crcf_destroy(resampler); }
 ...
 */
void syncer::process(shared_ptr<vector<complex<float>>> &samples,
                     int64_t metadata) {
  SPDLOG_DEBUG("Received {} samples", samples.get()->size());

  // Apply frequency correction to new samples
  SPDLOG_DEBUG(
      "Applying CFO {} to new samples coming to the processing queue counter",
      -cfo);
  ...
  if (state == state::wait) {
    if ((waiting_for_pss > sample_rate * ssb_period) &
        (waiting_for_pss - processing_queue.size() <
         sample_rate *
             ssb_period)) { // We have to account for the previous chunk of 8 ms
                            // where we found SSB and we send already and then
                            // we started counting after that.
      state = state::find_pss;
    } else {
      int sent_samples_size = processing_queue.size();
      shared_ptr<vector<complex<float>>> processing_queue_ptr =
          make_shared<vector<complex<float>>>(
              std::move(processing_queue)); // TODO allocate processing_queue on
                                            // the heap in the first place so we
                                            // don't need to do this copy
 ...
 */
void syncer::downsample(uint64_t num_samples, int64_t start_sample,
                        int64_t end_sample) {
  for (int64_t i = start_sample; i < end_sample; i++) {
    complex<float>
        temp[max_resampled_samples_per_sample]; // Temporary buffer for each
...
void syncer::find_pss() {
  ...
  uint64_t ssb_num_samples =
      ceilf(ssb_num_samples_downsampled / resampling_rate);

  // We want 10 symbols to have sufficient size for the fine sync correlation
  if (processing_queue.size() > ssb_num_samples) {
    // Downsample the signal to span the SSB subcarriers and push to internal
    // buffer
    downsampled_samples.clear();
    ...
    // Only look for PSS in a window of samples, as we already have a coarse
    // estimation where it will be
    if (phy->in_synch) {
      start_sample =
          max(int64_t(0), int64_t((sample_rate * ssb_period -
                                   (waiting_for_pss - processing_queue.size()) -
                                   pss_window_size)));
      end_sample = min(int64_t(processing_queue.size()),
                       int64_t(start_sample + pss_window_size * 2 + 1));
      downsampled_offset = (start_sample + 1) * resampling_rate;
    } else {
      start_sample = 0;
      end_sample =
          processing_queue.size(); // Look for SSB only in the first half for
                                   // speed during initial synch.
      downsampled_offset = 0;
    }
    downsample(processing_queue.size() - ssb_num_samples, start_sample,
               end_sample);
    // Dot product with the possible PSS
    bool pss_found = false;
    uint64_t timing_error = 0;
    uint64_t timing_error_downsampled_offset = 0;
    float max_corr = 0.0;
    uint8_t nid2 = 0;
    for (uint8_t pss_idx = pss_start; pss_idx <= pss_end; pss_idx++) {
      ...
      // Correlate
      int step_size = 1;
      correlate_magnitude(correlation_magnitudes, downsampled_samples,
                          {psss[pss_idx].get_pss_seq_t().data(),
                           psss[pss_idx].get_pss_seq_t().size()},
                          step_size);

      // Get the average
      float avg_correlation = 0.0f;
      volk_32f_accumulator_s32f(&avg_correlation, correlation_magnitudes.data(),
                                correlation_magnitudes.size());
      avg_correlation /= (float)correlation_magnitudes.size();

      // Find a point larger than x correlation. 0.1 for normalized often
      // appears to sync to other cells but is still a reliable indicator of PSS
      // being present, so kept it for now.
      float threshold = pss_sss_times_avg_threshold * avg_correlation;
      for (size_t i = 0; i < correlation_magnitudes.size(); i++) {
        float abs_correlation = std::abs(correlation_magnitudes.at(
            i)); // We allow correlation to be negative since we are not
    ...
    if (pss_found) {
      // Set PHY NID2
      phy->nid2 = nid2;
      SPDLOG_DEBUG("PSS found max corr {} Setting PHY NID2 to {:}", max_corr,
                   phy->nid2);

      // Cut the received samples at the correct position (both normal and
      // downsampled) We want to cut right before the CP of the PSS (second
      // symbol).
      ...
      downsampled_samples.erase(downsampled_samples.begin(),
                                downsampled_samples.begin() + timing_error);
      downsampled_samples.erase(downsampled_samples.begin() +
                                    ssb_num_samples_downsampled,
                                downsampled_samples.end());
      ...
      uint64_t sss_start_offset = phy->ssb_bwp->samples_per_symbol(2) +
                                  phy->ssb_bwp->samples_per_symbol(3) +
                                  phy->ssb_bwp->samples_per_cp(4);
      int64_t downsampled_sss_hint =
          timing_error_downsampled_offset + sss_start_offset;
      sss_hint = static_cast<uint64_t>(
          ((float)downsampled_sss_hint / resampling_rate));
      state = state::fine_sync;
    } else {
      SPDLOG_DEBUG("PSS Not Found, sending to next worker all except a window "
                   "at the end in case PSS is in between chunks");
...
void syncer::fine_sync() {
  ...
  moving_correlate(
      correlations, {downsampled_samples.data(), downsampled_samples.size()},
      {delayed_samples.data(), downsampled_samples.size()}, cp_length);
  ...
  complex<float> average =
      correlations[symbol_length * 1] + correlations[symbol_length * 2] +
      correlations[symbol_length * 3] + correlations[symbol_length * 4];

  new_cfo_fine =
      phy->ssb_bwp->scs * (std::arg(average) / (2 * std::numbers::pi));
  SPDLOG_DEBUG("NEW CFO fine (Hz): {}", new_cfo_fine);

  // Perform the new CFO immediately.
  rotate(downsampled_samples, downsampled_samples, -new_cfo_fine,
         phy->ssb_bwp->sample_rate);
...
void syncer::find_sss() {
  ...
  ofdm.symbol_index =
      2; // We are aligned to the PSS, which is the 3rd symbol in the SSB.
  auto ssb_mapper = make_shared<class ssb_mapper>(phy);

  // Set block callbacks
  ssb_mapper->on_sss_found =
      std::bind(&syncer::on_sss_found, this, std::placeholders::_1);
  ssb_mapper->on_sss_not_found = std::bind(&syncer::on_sync_lost, this);
  ssb_mapper->pbch.on_mib_found =
      std::bind(&syncer::on_mib_found, this, std::placeholders::_1,
                std::placeholders::_2);
  ...
  auto downsampled_samples_ptr = make_shared<vector<complex<float>>>(
      downsampled_samples); // TODO allocate downsampled_samples on the heap in
                            // the first place so we don't need to do this copy
...
void syncer::on_mib_found(srsran_mib_nr_t &mib, bool found) {
  if (found == false) {
    ...
    SPDLOG_INFO("Got MIB\nSSB \n Cell ID: {} \n MIB: SFN: {}, SCS: {} ",
                phy->get_cell_id(), mib.sfn, (int)mib.scs_common);
    mib_id++;
    char mib_str[512] = {};
    ...
    if (phy->bandwidth_parts.size() == 0) {
      ...
      for (pdcch_config pdcch_cfg : config.pdcch_configs) {
        ...
        if (pdcch_cfg.use_config_from_mib) {
          ...
          std::array<uint8_t, 4> coreset0_config =
              phy->ssb_bwp->get_pdcch_coreset0(5, phy->ssb_bwp->scs,
                                               15000U << mib.scs_common,
                                               mib.coreset0_idx);
          pdcch_cfg.num_prbs = coreset0_config.at(1);
          pdcch_cfg.coreset_duration = coreset0_config.at(2);
          pdcch_cfg.subcarrier_offset =
              mib.ssb_offset + (pdcch_cfg.num_prbs / 2);
          pdcch_cfg.extended_prefix = false;
        }

        // Simplify brute force if we are looking only for SI DCI
        if (pdcch_cfg.si_dci_only) {
          pdcch_cfg.scrambling_id_start =
              phy->get_cell_id(); // Scrambling ID for SI DCI is always the cell
                                  // ID
        ...
        auto new_bwp = make_shared<bandwidth_part>(
            this->sample_rate, pdcch_cfg.numerology, pdcch_cfg.num_prbs,
            pdcch_cfg.extended_prefix);
    ...
    // If we had at least one BWP, perform fine time synchronization on the
    // first BWP added (full-rate)
    if (phy->bandwidth_parts.size() > 0) {
      fine_time_sync();
    }
    ...
    for (size_t i = 0; i < phy->bandwidth_parts.size(); i++) {
      auto bwp = phy->bandwidth_parts.at(i);
      auto mapper = phy->channel_mappers.at(i);
      auto flow = flow_pool->acquire_flow();
      auto rotator = make_shared<class rotator>(
          this->sample_rate,
          (float)mapper->pdcch.subcarrier_offset * (float)bwp->scs);
      auto ofdm = make_shared<class ofdm>(bwp);
      flow->connect(rotator);
      rotator->connect(ofdm);
      ofdm->connect(
          mapper); // Connect OFDM block to the shared PHY-layer channel mapper
    }
...
void syncer::fine_time_sync() {
  ...
  // TODO this is actually OFDM modulation. Make OFDM modulator block and add
  // PSS / PBCH DMRS as well to improve correlation
  auto sss_samples_f = phy->ssss.at((phy->nid1) * 3 + phy->nid2);
  sss_full_rate.insert(sss_full_rate.begin() + start_offset,
                       sss_samples_f.begin(), sss_samples_f.end());
  sss_full_rate.resize(initial_bwp->fft_size);

  // FFT shift
  std::rotate(sss_full_rate.begin(),
              sss_full_rate.begin() + sss_full_rate.size() / 2,
              sss_full_rate.end());
  ...
  fftplan q =
      fft_create_plan(initial_bwp->fft_size, sss_full_rate.data(),
                      sss_full_rate_time.data(), LIQUID_FFT_BACKWARD, flags);
  fft_execute(q);
  fft_destroy_plan(q);
  ...
  uint64_t search_space_size =
      initial_bwp->samples_per_symbol(4) * 2; // Search max 2 symbols

  correlate_magnitude(
      correlation_magnitudes,
      {processing_queue.data() + search_space_start, search_space_size},
      sss_full_rate_time);
  for (size_t i = 0; i < correlation_magnitudes.size(); i++) {
  ...
  sss_position +=
      search_space_start; // Offset is relative to search space start, so true
                          // position needs to account for this

  uint64_t sss_ref_position =
      initial_bwp->samples_per_symbol(0) + initial_bwp->samples_per_symbol(1) +
      initial_bwp->samples_per_symbol(2) + initial_bwp->samples_per_symbol(3) +
      initial_bwp->samples_per_cp(4);
  // In the timing error we also move slightly (1%) into the CP to the left
  // (assuming channel length < CP), this is usually done to avoid ISI/ICI from
  // next symbol
  int64_t timing_error = sss_position - sss_ref_position -
                         std::floor(0.01 * initial_bwp->samples_per_cp(0));
  SPDLOG_DEBUG("Full-rate samples per symbol: {}",
               initial_bwp->samples_per_symbol(1));
  ...
  if (timing_error > 0) {
    ...
    std::copy(processing_queue.begin(), processing_queue.begin() + timing_error,
              queue_remainder.begin());

    // Send the part that will be erased from the processing queue to any
    // existing flows, so they can still process these samples
    shared_ptr<vector<complex<float>>> processing_queue_remainder =
        make_shared<vector<complex<float>>>(std::move(queue_remainder));
    ...
    // Tell all currently existing flows to finish processing after the workload
    // they received now
    this->flow_pool->release_flows();

    // Cut the processing queue
    processing_queue.erase(processing_queue.begin(),
                           processing_queue.begin() + timing_error);
  } else {
    // If the correlation is higher for negative offsets, we need to append
    // samples
