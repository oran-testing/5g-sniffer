...
struct config {
  ...
  static struct config load(string config_path) {
    ...
    printf("Loading configuration file %s", config_path.c_str());
    toml::table toml = toml::parse_file(config_path);
    conf.file_path = toml["sniffer"]["file_path"].value_or(""sv).data();
    conf.sample_rate =
        toml["sniffer"]["sample_rate"].value_or(default_sample_rate);
    ...
    if (!toml["pdcch"].is_array_of_tables())
      throw config_exception(
          "PDCCH TOML config should be an array of tables, e.g. [[pdcch]]");
    ...
    for (toml::node &node : *pdcch_tables) {
      if (node.is_table()) {
        ...
        pdcch_cfg.use_config_from_mib =
            pdcch_table["use_config_from_mib"].value_or(false);
        pdcch_cfg.coreset_id = pdcch_table["coreset_id"].value_or(0);
        pdcch_cfg.subcarrier_offset =
            pdcch_table["subcarrier_offset"].value_or(0);
        ...
        pdcch_cfg.extended_prefix =
            pdcch_table["extended_prefix"].value_or(false);
        ...
        pdcch_cfg.scrambling_id_start =
            pdcch_table["scrambling_id_start"].value_or(0);
        pdcch_cfg.scrambling_id_end =
            pdcch_table["scrambling_id_end"].value_or(0xffff);
        pdcch_cfg.max_rnti_queue_size =
            pdcch_table["max_rnti_queue_size"].value_or(0xffff);
        pdcch_cfg.sample_rate_time = conf.sample_rate;
        pdcch_cfg.rnti_list_length =
            pdcch_table["rnti_list_length"].value_or(0xffff);
        toml::array *dci_array =
            pdcch_table["dci_sizes_list"].as<toml::array>();
        // Parse the DCI array list and if is not included, add 39 by default
        ...
        toml::array *AL_thresholds_array =
            pdcch_table["AL_corr_thresholds"].as<toml::array>();
        if (AL_thresholds_array) {
          printf("AL corr in the toml");
          for (auto &&elem : *AL_thresholds_array) {
            printf("AL corr in the toml element %f", elem.value_or(1.0));
        ...
        toml::array *num_candidates_per_AL_array =
            pdcch_table["num_candidates_per_AL"].as<toml::array>();
        if (num_candidates_per_AL_array) {
          printf("num_candidates_per_AL in the toml, size is %ld",
                 num_candidates_per_AL_array->size());
          for (auto &&elem : *num_candidates_per_AL_array) {
            printf("num_candidates_per_AL in the toml element %d",
                   elem.value_or(1));
        ...
        pdcch_cfg.sc_power_decision =
            pdcch_table["sc_power_decision"].value_or(false);
        pdcch_cfg.coreset_interleaving_pattern =
            pdcch_table["coreset_interleaving_pattern"].value_or(
                "non-interleaved");
        pdcch_cfg.coreset_duration =
            pdcch_table["coreset_duration"].value_or(1);
        pdcch_cfg.coreset_ofdm_symbol_start =
            pdcch_table["coreset_ofdm_symbol_start"].value_or(0);
        pdcch_cfg.coreset_nshift = pdcch_table["coreset_nshift"].value_or(0);
        pdcch_cfg.coreset_reg_bundle_size =
            pdcch_table["coreset_reg_bundle_size"].value_or(6);
        pdcch_cfg.coreset_interleaver_size =
            pdcch_table["coreset_interleaver_size"].value_or(2);
